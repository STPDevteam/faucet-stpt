...
import { DAO_FACTORY_ADDRESS, STPT_AIRDROP_ADDRESS } from '../constants'
import STPT_AIRDROP_ABI from '../constants/abis/STPT_airdrop.json'
...
export function useSTPTAirdropContract(): Contract | null {
  const { chainId } = useActiveWeb3React()
  return useContract(
    chainId && STPT_AIRDROP_ADDRESS[chainId] ? STPT_AIRDROP_ADDRESS[chainId] : undefined,
    STPT_AIRDROP_ABI,
    true
  )
}
